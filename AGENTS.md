...
**Supporting skills:** foundry-refactor (LSP-powered), foundry-note (capture ideas), foundry-research (AI consultation)
...
**MCP-First**: All skills use Foundry MCP tools exclusively. Never read spec JSON files directly with file read tools or shell commands.

**Human-in-Loop**: Skills prompt for key decisions. Don't assume - ask.

**Task States**: `pending` → `in_progress` → `completed` | `blocked`
- Only one task `in_progress` at a time (sequential mode)
...
Launch the exploration agent before skill invocation when you need:
...
Example: Before `foundry-spec`, explore to understand existing architecture.

## Spec Locations

| Location | Purpose |
|----------|---------|
| `specs/pending/` | Specs being authored |
| `specs/active/` | Specs being implemented |
| `specs/completed/` | Finished specs |
| `specs/.plans/` | Markdown plans before JSON |
