...
1. Call `foundry-mcp_health action="liveness"` to verify MCP server connectivity
2. Call `foundry-mcp_environment action="verify-toolchain"` to check required tools (Python, git)
3. Call `foundry-mcp_environment action="verify-env"` to validate runtime versions
...
OpenCode requires explicit permissions for plugin tools. This phase automatically detects, compares, and configures the required permissions.
...
Explain:
> "Found `opencode.json` but it contains invalid JSON. The file will be backed up before replacement."
...
2. Call `foundry-mcp_spec action="list"` to check for existing specifications
...
If yes: Call `foundry-mcp_environment action="init-workspace"` to create:
- specs/active/
- specs/pending/
- specs/completed/
- specs/archived/
...
If yes: Call `foundry-mcp_environment action="setup"` with `dry_run=false`

Report what was created or skipped.

---

## Phase 2.1: Implement Configuration
...
## Phase 2.2: AI Provider Configuration
...
Call `foundry-mcp_provider action="list"` to get all registered providers.
...
```
foundry-mcp_provider action="status" provider_id="<provider_id>"
```
...
## Phase 2.3: Research Configuration
...
Use the available providers from Phase 2.2 to configure research defaults.

**Format:** Use the same format as consultation: `[cli]provider:model`

Reuse the priority entries already built in Phase 2.2 for the consultation section.
...
Call `foundry-mcp_environment action="detect-test-runner"` to analyze the project.
...
## Phase 3: AGENTS.md Configuration

This phase creates or updates the project's AGENTS.md file with Foundry SDD workflow guidance. This helps ensure OpenCode uses the correct skills and tools for spec-driven development.

### Step 1: Check for Existing AGENTS.md

Use the Read tool to check if `AGENTS.md` exists in the project root.
...
Use `AskUserQuestion`:
- Question: "Create AGENTS.md with Foundry SDD workflow guidance?"
- Options:
...
**If AGENTS.md exists but no marker:**

Use `AskUserQuestion`:
- Question: "Append Foundry SDD section to existing AGENTS.md?"
- Options:
...
Display:
> "Foundry SDD guidance already present in AGENTS.md. Skipping."
...
- Do NOT call `foundry-mcp_authoring` directly
- Do NOT call `foundry-mcp_task` directly
...
**If created:**
> "Created AGENTS.md with Foundry SDD workflow guidance."

**If appended:**
> "Appended Foundry SDD section to existing AGENTS.md."

**If skipped:**
> "AGENTS.md configuration skipped."
...
> "**Restart OpenCode** for the permission changes to take effect."
