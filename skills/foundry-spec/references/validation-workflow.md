...
```bash
# Preview fixes first
foundry-mcp_spec action="fix" spec_id="{spec-id}" dry_run=true
...
```bash
foundry-mcp_spec action="stats" spec_id="{spec-id}"
```
...
```bash
foundry-mcp_spec action="analyze-deps" spec_id="{spec-id}" bottleneck_threshold=3
```
...
```bash
foundry-mcp_spec action="completeness-check" spec_id="{spec-id}"
```
...
```bash
foundry-mcp_spec action="duplicate-detection" spec_id="{spec-id}"
```
...
```bash
foundry-mcp_spec action="diff" spec_id="{spec-id}" compare_to="{backup-path-or-version}"
```
...
```bash
foundry-mcp_spec action="validate" spec_id="my-spec-001"
```

**Path:**
```bash
foundry-mcp_spec action="validate" path="specs/pending/my-spec-001.json"
```
