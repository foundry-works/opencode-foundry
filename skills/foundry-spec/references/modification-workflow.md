...
- Apply review feedback from foundry-spec review
...
**Do NOT use for:**
- Status tracking or task completion (use `foundry-implement`)
- Creating new specifications (during initial planning)
- Finding next actionable task (use `foundry-implement`)
...
```bash
foundry-mcp_review action="parse-feedback" spec_id="my-spec-001" review_path="reports/plan-review.md"
```
...
```bash
foundry-mcp_spec action="apply-plan" spec_id="my-spec-001" modifications_file="suggestions.json" dry_run=true
```
...
```bash
foundry-mcp_spec action="apply-plan" spec_id="my-spec-001" modifications_file="suggestions.json"
```
...
foundry-mcp_task action="move" spec_id="my-spec-001" task_id="task-2-3" position=1

# Move task to different phase
foundry-mcp_task action="move" spec_id="my-spec-001" task_id="task-2-3" parent="phase-3" position=1
```
...
foundry-mcp_authoring action="phase-move" spec_id="my-spec-001" phase_id="phase-3" position=1
```
...
foundry-mcp_authoring action="spec-find-replace" spec_id="my-spec-001" find="old-api" replace="new-api" scope="all"

# Replace only in titles
foundry-mcp_authoring action="spec-find-replace" spec_id="my-spec-001" find="v1" replace="v2" scope="titles"
```
...
foundry-mcp_spec action="history" spec_id="my-spec-001"

# Preview rollback
foundry-mcp_authoring action="spec-rollback" spec_id="my-spec-001" version="2025-12-27T20-54-48.149929" dry_run=true

# Apply rollback
foundry-mcp_authoring action="spec-rollback" spec_id="my-spec-001" version="2025-12-27T20-54-48.149929"
```
...
- Continue with foundry-implement if ready for implementation
```
