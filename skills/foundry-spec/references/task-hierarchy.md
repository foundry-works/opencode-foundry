...
foundry-mcp_task action="move" spec_id="{spec-id}" task_id="task-2-3" parent="phase-1"

# Move task to specific position within parent
foundry-mcp_task action="move" spec_id="{spec-id}" task_id="task-1-3" parent="phase-1" position=0

# Preview move without applying
foundry-mcp_task action="move" spec_id="{spec-id}" task_id="task-2-3" parent="phase-1" dry_run=true
```
...
foundry-mcp_task action="add-dependency" spec_id="{spec-id}" task_id="task-2-1" depends_on="task-1-3"

# Add soft dependency
foundry-mcp_task action="add-dependency" spec_id="{spec-id}" task_id="task-2-1" depends_on="task-1-3" dependency_type="soft"
```
...
```bash
# Remove a dependency
foundry-mcp_task action="remove-dependency" spec_id="{spec-id}" task_id="task-2-1" depends_on="task-1-3"
```
...
foundry-mcp_task action="add-requirement" spec_id="{spec-id}" task_id="task-1-2" requirement="API returns 200 OK for valid requests"

# Add multiple requirements
foundry-mcp_task action="add-requirement" spec_id="{spec-id}" task_id="task-1-2" requirement="Error responses include actionable messages"
```
...
foundry-mcp_task action="add-dependency" spec_id="my-spec" task_id="task-1-2" depends_on="task-1-1"
```

**Scenario 2: Multiple prerequisites**
```bash
# Task 3 requires both Task 1 and Task 2
foundry-mcp_task action="add-dependency" spec_id="my-spec" task_id="task-1-3" depends_on="task-1-1"
foundry-mcp_task action="add-dependency" spec_id="my-spec" task_id="task-1-3" depends_on="task-1-2"
```

**Scenario 3: Cross-phase dependency**
```bash
# Phase 2 task depends on Phase 1 task
foundry-mcp_task action="add-dependency" spec_id="my-spec" task_id="task-2-1" depends_on="task-1-3"
```

> Tip: After modifying dependencies, run `foundry-mcp_spec action="analyze-deps"` to detect cycles or orphans.
