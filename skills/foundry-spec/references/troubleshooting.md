...
foundry-mcp_spec action="validate-fix" spec_id="{spec-id}" auto_fix=true
```
...
foundry-mcp_authoring action="phase-template" template_action="apply" template_name="testing" spec_id="{spec-id}"
```
...
foundry-mcp_authoring action="phase-add-bulk" spec_id="{spec-id}" phase='{"title": "Phase Title"}' tasks='[{"type": "task", "title": "Task 1", "description": "Explain the task", "task_category": "investigation", "acceptance_criteria": ["Notes captured in spec journal"]}]'
...
foundry-mcp_authoring action="phase-add-bulk" spec_id="{spec-id}" phase='{"title": "Test Phase"}' tasks='[{"type": "task", "title": "Test task"}]' dry_run=true
```
