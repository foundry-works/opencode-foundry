...
This skill delegates all fidelity review logic to the dedicated `foundry-mcp_review action="fidelity"` MCP tool, which handles spec loading, implementation analysis, AI consultation, and report generation.
...
Build the appropriate `foundry-mcp_review action="fidelity"` command based on review scope:
...
- Use `foundry-mcp_task action="query" spec_id={spec-id} parent={phase-id}` to get all tasks in a phase
- Use `foundry-mcp_task action="query" spec_id={spec-id} status={status}` to filter by status
- Use `foundry-mcp_task action="info" spec_id={spec-id} task_id={task-id}` to get a single task's details
- Use `foundry-mcp_task action="list" spec_id={spec-id} include_phases=true` to see all phases
