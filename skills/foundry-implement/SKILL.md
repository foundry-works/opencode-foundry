...
This skill interacts solely with the Foundry MCP server (`foundry-mcp`). Tools use the router+action pattern: `foundry-mcp_<router>` with `action="<action>"`.
...
- `foundry-mcp_task action="prepare"`
- `foundry-mcp_task action="info"`
- `foundry-mcp_spec action="find"`
- `foundry-mcp_task action="query"`
...
- **Recommendation path**: `foundry-mcp_task action="prepare"` -> surface task id, file, estimates, blockers
- **Browsing path**: Use `foundry-mcp_task action="query"` -> present shortlist via `AskUserQuestion`

### Task Type Dispatch

After selecting a task, check its type to determine the workflow path.

**Check task type** from the `task action="prepare"` response or via `foundry-mcp_task action="info"`:
...
Invoke `foundry-mcp_task action="prepare"` with the target `spec_id`. The response contains:
...
Treat `context` as the authoritative source. Only fall back to `foundry-mcp_task action="info"` when the spec explicitly requires absent data.
...
Before implementing, use OpenCode's built-in subagents for efficient codebase exploration:
...
**Before coding:**
```bash
foundry-mcp_task action="update-status" spec_id={spec-id} task_id={task-id} status="in_progress" note="context"
```
...
```bash
foundry-mcp_task action="complete" spec_id={spec-id} task_id={task-id} completion_note="Summary of what was accomplished, tests run, verification performed"
```
...
- Document blocker using `foundry-mcp_task action="block"`
...
foundry-mcp_task action="add-dependency" spec_id={spec-id} task_id={task-id} depends_on={dependency-task-id}

# Discovered a new acceptance requirement (e.g., from testing)
foundry-mcp_task action="add-requirement" spec_id={spec-id} task_id={task-id} requirement="Description of discovered requirement"
```
...
```bash
foundry-mcp_task action="unblock" spec_id={spec-id} task_id={task-id} resolution="Brief description"
```
...
```bash
foundry-mcp_task action="complete" spec_id="my-spec-001" task_id="task-1-2" completion_note="Implemented phase-add-bulk handler. Basic verification: imports work, no syntax errors. Full test run deferred to verify-1-1. Created src/tools/authoring.py handler (200 lines)."
```

**Example (without sibling verify task - full testing):**
```bash
foundry-mcp_task action="complete" spec_id="my-spec-001" task_id="task-2-3" completion_note="Implemented JWT auth middleware with PKCE flow. All 12 unit tests passing. Manual verification: login flow works in dev environment. Created src/middleware/auth.ts (180 lines) and tests/middleware/auth.spec.ts (45 tests)."
```
