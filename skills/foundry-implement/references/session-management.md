...
```bash
foundry-mcp_task action="session-config" \
  spec_id={spec-id} \
  command="start"
```
...
foundry-mcp_task action="session-config" command="start" ...
```

**Right:**
```bash
# Always check status first
foundry-mcp_task action="session-config" command="status" ...
...
**Wrong:**
```
Session paused. Continuing anyway...
...
```
Session paused.
Resolve the cause, then `foundry-implement --auto` to resume safely.
```
...
foundry-mcp_task action="session-config" command="resume" ...
...
foundry-mcp_task action="unblock" ...
# Or skip the blocked task
foundry-mcp_task action="update-status" status="blocked" ...
# Then resume
foundry-mcp_task action="session-config" command="resume" ...
```
...
**Wrong:**
```
# Starting a large multi-file task without checkpointing
```

**Right:**
```
# Checkpoint before large tasks
# Pause proactively before starting if needed
```
