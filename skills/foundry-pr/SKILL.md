...
The `foundry-pr` skill creates professional, comprehensive pull requests by analyzing your completed specs and git history. Instead of manually writing PR descriptions, this skill uses AI to analyze all available context and generate detailed, well-structured PR descriptions.

## When to Use This Skill

Use `Skill(foundry:foundry-pr)` to:
...
This skill works exclusively through the Foundry MCP server (`foundry-mcp`). Tools use the router+action pattern: `foundry-mcp_<router>` with `action="<action>"`.

**Key tools:**
- `foundry-mcp_pr action="create"` - Create PR with spec context
- `foundry-mcp_pr action="context"` - Gather PR context from spec
- `foundry-mcp_journal action="list"` - Get journal entries for context
- `foundry-mcp_spec action="history"` - View spec evolution timeline
- `foundry-mcp_spec action="diff"` - Compare spec versions
...
foundry-mcp_spec action="history" spec_id="{spec-id}" limit=10

# Compare current spec vs original to show requirement evolution
foundry-mcp_spec action="diff" spec_id="{spec-id}" compare_to="specs/.backups/{spec-id}-original.json"
```
...
Traditional PR creation uses static templates. The foundry-pr skill:
...
Skill(foundry:foundry-pr) "Create PR for spec oauth-feature-2025-11-03-001"
