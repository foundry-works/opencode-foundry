...
```json
{
  ...
  "verification": {
    ...
    "mcp_tool": "foundry-mcp_test action=\"run\"",
...
**MCP invocation:**
```bash
foundry-mcp_authoring action="phase-update-metadata" spec_id={spec-id} phase_id="phase-2" description="Updated description" estimated_hours=8.5
```
...
**MCP invocation:**
```bash
foundry-mcp_task action="move" spec_id={spec-id} task_id="task-2-3" parent="phase-3" position=1
```
...
**MCP invocation:**
```bash
foundry-mcp_authoring action="phase-move" spec_id={spec-id} phase_id="phase-2" position=1
```
...
**MCP invocation:**
```bash
foundry-mcp_authoring action="spec-find-replace" spec_id={spec-id} find="old-pattern" replace="new-value" scope="all"
```
