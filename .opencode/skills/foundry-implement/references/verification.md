...
```bash
foundry-mcp_task action="info" spec_id={spec-id} task_id={task-id}
```
...
**If verdict = pass:**
```bash
foundry-mcp_task action="complete" spec_id={spec-id} task_id={task-id} completion_note="Fidelity review passed with verdict: {verdict}."
```
